...
const { usdcContract, uniV2RouterContract, uniV2FactoryContract, compoundContract, } = require("./externalContracts")

const { usdc, weth, convexBooster, compoundPid, curveCompound, compoundLP, triLP, crv, stkCOMPOUND, lqty, alcx, uniSwapV2Router, crvUsdcPath } = require("../constants/constants")

let vault, controller, compound, depositApprover, exchange
...
describe("ENF Vault test", async () => {
    ...
    // it("Get Pid", async () => {
    //     const triPID = await compound.getPID(triLP)
    ...
    it("Emergency Withdraw by non-owner will be reverted", async () => {
        await expect(compound.connect(alice).emergencyWithdraw()).to.be.revertedWith("Ownable: caller is not the owner")
    })

    it("Emergency Withdraw", async () => {
        await compound.emergencyWithdraw()
    })

    // it("Get LP withdrawn", async () => {
    //     const lpBal = await compoundContract(alice).balanceOf(deployer.address)
    //     console.log(`\tCompound LP Withdrawn: ${toEth(lpBal)}`)
    // })
    ...
    it("Owner deposit will be reverted", async () => {
        await expect(compound.connect(alice).ownerDeposit(fromUSDC(100))).to.revertedWith("Ownable: caller is not the owner")
    })
    ...
    it("Owner Deposit", async () => {
        ...
        // Read Total Assets
        const total = await compound.totalAssets()
